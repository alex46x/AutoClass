...
export default function AdminShiftStudentModal({ studentId, studentName, currentSectionId, sections }: {
  studentId: number; studentName: string; currentSectionId: number | null; sections: Section[];
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adminShiftStudent(studentId, parsedId);
      setIsOpen(false);
    } catch (err: any) {
  ...
  return (
    <>
      <button onClick={() => setIsOpen(true)}
        className="p-1.5 text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors" title="Shift Student">
