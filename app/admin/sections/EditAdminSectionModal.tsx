...
import { editAdminSection } from '@/app/actions/admin';
import { motion, AnimatePresence } from 'framer-motion';

export default function EditAdminSectionModal({ sectionId, currentName, currentMaxStudents }: { sectionId: number; currentName: string; currentMaxStudents: number }) {
  const [isOpen, setIsOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [name, setName] = useState(currentName);
  const [maxStudents, setMaxStudents] = useState<number>(currentMaxStudents);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    try {
      await editAdminSection(sectionId, name, maxStudents);
      setIsOpen(false);
    } catch (err: any) {
  ...
  return (
    <>
      <button onClick={() => setIsOpen(true)}
        className="p-1.5 text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors" title="Edit Section">
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  <BookOpen className="w-5 h-5 text-indigo-500" /> Edit Section
                </h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input required type="number" min="1" max="200" value={maxStudents}
