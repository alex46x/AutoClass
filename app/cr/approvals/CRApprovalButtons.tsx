...
import { approveClassmate, rejectClassmate } from '@/app/actions/cr';
import { Check, X } from 'lucide-react';

export default function CRApprovalButtons({ userId }: { userId: number }) {
  ...
  const handleAction = async (status: 'ACTIVE' | 'REJECTED') => {
    ...
    try {
      if (status === 'ACTIVE') {
        await approveClassmate(userId);
      } else {
        await rejectClassmate(userId);
      }
    } catch (e: any) {
